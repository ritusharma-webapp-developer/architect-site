...
import { getFirestore, collection, addDoc, serverTimestamp, getDocs, query, orderBy, onSnapshot } from "https://www.gstatic.com/firebasejs/9.22.0/firebase-firestore.js";
...
function applyFilter(filterValue) {
    ...
    portfolioItems.forEach(item => {
        const categoryElement = item.querySelector('.portfolio-overlay p');
        ...
        if (filterValue === 'all' || category === filterValue) {
            ...
            // Only animate if it's not the initial "all" load
            if (filterValue !== 'all') {
...
// Load Portfolio from Firebase (Optimized with onSnapshot)
function loadPortfolio() {
    const portfolioGrid = document.querySelector('.portfolio-grid');
    if (!portfolioGrid) return;

    const q = query(collection(db, "portfolio-items"), orderBy("createdAt", "desc"));
    
    // Use onSnapshot for real-time updates and faster initial appearance
    onSnapshot(q, (querySnapshot) => {
        console.log("Portfolio Snapshot received, size:", querySnapshot.size);
        // Clear existing items (including loading spinner)
        portfolioGrid.innerHTML = '';

        if (querySnapshot.empty) {
            portfolioGrid.innerHTML = `
                <div class="text-center" style="grid-column: 1/-1; padding: 60px; color: #666;">
                    <i class="fa-regular fa-folder-open" style="font-size: 40px; margin-bottom: 20px; display: block;"></i>
                    <p>No projects found in this gallery yet.</p>
                </div>
            `;
            return;
        }
        ...
        querySnapshot.forEach((doc) => {
            ...
            itemDiv.className = `portfolio-item ${data.isLarge ? 'item-large' : ''}`;
            
            // Optimized image loading with loading="lazy" and fade-in
            itemDiv.innerHTML = `
                <img src="${data.imageUrl}" alt="${data.title}" loading="lazy" style="opacity: 0; transition: opacity 0.5s ease-in-out;" onload="this.style.opacity='1'">
        ...
        console.log("Portfolio synced from Firebase");
        // Re-apply filter if an active filter is NOT 'all'
        const activeFilterBtn = document.querySelector('.filter-btn.active');
        if (activeFilterBtn && activeFilterBtn.getAttribute('data-filter') !== 'all') {
            applyFilter(activeFilterBtn.getAttribute('data-filter'));
        }
    }, (error) => {
        console.error("Error syncing portfolio: ", error);
        portfolioGrid.innerHTML = `<p class="text-center" style="grid-column: 1/-1; color: #f87171;">Failed to load projects. Please try again later.</p>`;
    });
}
